...
impl HeaderAnalysisPlugin {
    ...
    fn check_csp_issues(&self, directives: &HashMap<String, Vec<String>>) -> Vec<String> {
        ...
        if !directives.contains_key("frame-ancestors") {
            issues.push("CSP missing frame-ancestors directive (clickjacking protection)".to_string());
        }
                ...
                if part.starts_with("max-age=") {
                    if let Ok(age) = part[8..].parse::<u64>() {
                        ...
                        if age < 31536000 { // Less than 1 year
                            analysis.issues.push("HSTS max-age less than 1 year (31536000 seconds)".to_string());
                        }
                    }
    ...
    fn analyze_permissions_policy(&self, headers: &HashMap<String, String>) -> PermissionsPolicyAnalysis {
    ...
    fn analyze_x_content_type_options(&self, headers: &HashMap<String, String>) -> XContentTypeOptionsAnalysis {
    ...
    fn analyze_x_xss_protection(&self, headers: &HashMap<String, String>) -> XXssProtectionAnalysis {
    ...
    fn analyze_cross_origin_policies(&self, headers: &HashMap<String, String>) -> CrossOriginPoliciesAnalysis {
...
impl Plugin for HeaderAnalysisPlugin {
    ...
    fn new(config: Self::Config) -> Self {
        Self::new(config).expect("Failed to create HeaderAnalysisPlugin")
    }
        ...
        if !analysis.hsts.present {
            ...
            for issue in &analysis.hsts.issues {
                findings.push(Finding::new(
                    ...
                    data: Some(serde_json::json!({"issue": issue, "hsts": analysis.hsts.value})),
                    location: Some(target_url.clone()),
                    metadata: HashMap::new(),
                }));
            }
