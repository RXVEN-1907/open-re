...
impl RobotsSitemapPlugin {
    ...
    async fn discover_sitemap(&self, base_url: &str, robots_sitemaps: &[String]) -> Result<SitemapResult> {
    ...
    async fn recon(&mut self, context: &ScanContext) -> Result<Vec<Finding>> {
        ...
        if robots.found {
            ...
            for path in &robots.parsed.disallowed_paths {
                if path.contains("admin") || path.contains("login") || path.contains("private") || path.contains("secret") {
                    findings.push(Finding::new(
        ...
        if sitemap.found {
            findings.push(Finding::new(
                ...
                format!("Sitemap discovered at: {}", sitemap.sitemap_url.as_deref().unwrap_or("unknown")),
                Severity::Info,
                Confidence::High,
                Category::InformationDisclosure,
                target_url.clone(),
        ...
        info!("Robots & sitemap discovery completed for: {} - {} findings", target_url, findings.len());
        Ok(findings)
    }
}
