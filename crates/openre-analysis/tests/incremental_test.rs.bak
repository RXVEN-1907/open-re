...
use openre_analysis::{Fingerprint, FingerprintIncrementalAnalyzer, StageMetrics, StageResult};
...
async fn test_fingerprint_change_detection() {
    ...
    let results1 = tokio::time::timeout(
    ...
    let results3 = tokio::time::timeout(
        std::time::Duration::from_secs(30),
        analyzer.analyze_if_changed(analysis_id, &binary_path, || {
            reanalyzed = true;
            let mut map = std::collections::HashMap::new();
...
async fn test_stage_level_invalidation() {
    ...
    tokio::time::timeout(
        std::time::Duration::from_secs(30),
        analyzer.invalidate_stage(&analysis_id, &StageId::new("disassembly")),
    )
    .await
    .unwrap();
