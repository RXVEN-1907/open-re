...
//! time-based, and union-based detection methods.
...
use crate::sdk::{Plugin, AnalysisContext, Result, Capability, CapabilityRequest, CapabilityResponse};
...
impl SqlInjectionPlugin {
    /// Create a new SQL injection plugin
    ...
    fn description(&self) -> &'static str {
        "Detects SQL injection vulnerabilities using error-based, boolean-based, time-based, and union-based detection methods"
    }
    ...
    fn references(&self) -> Vec<crate::injection::SecurityReference> {
        vec![
            ...
            crate::injection::SecurityReference {
                ref_type: "CWE".to_string(),
                id: "CWE-564".to_string(),
                url: "https://cwe.mitre.org/data/definitions/564.html".to_string(),
                description: "SQL Injection: Hibernate".to_string(),
            },
        ]
    }
...
impl Plugin for SqlInjectionPlugin {
    ...
    async fn execute(&self, request: CapabilityRequest) -> Result<CapabilityResponse> {
        ...
        info!("Starting SQL injection testing for {}", target_url);
        ...
        // Convert to findings
        let scan_id = context.job_id.into(); // This would need proper ScanId
...
impl crate::injection::InjectionPlugin for SqlInjectionPlugin {
    fn injection_category(&self) -> InjectionCategory {
        InjectionCategory::SqlInjection
    }
    ...
    fn response_analyzer(&self) -> Box<dyn ResponseAnalyzer> {
        create_response_analyzer(InjectionCategory::SqlInjection)
    }
}

// Plugin entry point
crate::plugin_entry!(SqlInjectionPlugin);