...
//!
//! Detects HTTP header injection vulnerabilities including response splitting,
//! cache poisoning, and header manipulation.
...
impl HeaderInjectionPlugin {
    /// Create a new Header injection plugin
    ...
    fn description(&self) -> &'static str {
        "Detects HTTP header injection vulnerabilities including response splitting, cache poisoning, and header manipulation"
    }
    ...
    fn references(&self) -> Vec<crate::injection::SecurityReference> {
        vec![
            ...
            crate::injection::SecurityReference {
                ...
                id: "CWE-113".to_string(),
                url: "https://cwe.mitre.org/data/definitions/113.html".to_string(),
                description: "Improper Neutralization of CRLF Sequences in HTTP Headers ('HTTP Response Splitting')".to_string(),
            },
            crate::injection::SecurityReference {
                ref_type: "CWE".to_string(),
                id: "CWE-79".to_string(),
...
#[async_trait]
impl Plugin for HeaderInjectionPlugin {
    type Config = InjectionPluginConfig;
    
    fn new(config: Self::Config) -> Self {
        Self::new(config).expect("Failed to create Header injection plugin")
    }
    ...
    async fn execute(&self, request: CapabilityRequest) -> Result<CapabilityResponse> {
        ...
        info!("Starting Header injection testing for {}", target_url);
...
impl crate::injection::InjectionPlugin for HeaderInjectionPlugin {
    fn injection_category(&self) -> InjectionCategory {
        InjectionCategory::HeaderInjection
    }
    ...
    fn response_analyzer(&self) -> Box<dyn ResponseAnalyzer> {
        create_response_analyzer(InjectionCategory::HeaderInjection)
    }
}

// Plugin entry point
crate::plugin_entry!(HeaderInjectionPlugin);