//! NoSQL Injection Plugin
//!
//! Detects NoSQL injection vulnerabilities in MongoDB and other NoSQL databases.
...
impl NoSqlInjectionPlugin {
    /// Create a new NoSQL injection plugin
    ...
    fn references(&self) -> Vec<crate::injection::SecurityReference> {
        vec![
            ...
            crate::injection::SecurityReference {
                ...
                id: "CWE-943".to_string(),
                url: "https://cwe.mitre.org/data/definitions/943.html".to_string(),
                description: "Improper Neutralization of Special Elements used in a NoSQL Query ('NoSQL Injection')".to_string(),
            },
        ]
    }
...
#[async_trait]
impl Plugin for NoSqlInjectionPlugin {
    type Config = InjectionPluginConfig;
    
    fn new(config: Self::Config) -> Self {
        Self::new(config).expect("Failed to create NoSQL injection plugin")
    }
    ...
    async fn execute(&self, request: CapabilityRequest) -> Result<CapabilityResponse> {
        ...
        info!("Starting NoSQL injection testing for {}", target_url);
        ...
        let payload_context = PayloadContext {
            ...
            location: ParameterLocation::Query,
            expected_type: None,
            technology_hints: vec!["mongodb".to_string()],
            database_type: Some("mongodb".to_string()),
...
impl crate::injection::InjectionPlugin for NoSqlInjectionPlugin {
    fn injection_category(&self) -> InjectionCategory {
        InjectionCategory::NoSqlInjection
    }
    ...
    fn response_analyzer(&self) -> Box<dyn ResponseAnalyzer> {
        create_response_analyzer(InjectionCategory::NoSqlInjection)
    }
}

// Plugin entry point
crate::plugin_entry!(NoSqlInjectionPlugin);