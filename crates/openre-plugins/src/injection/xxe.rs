//! XXE Injection Plugin
//!
//! Detects XML External Entity (XXE) vulnerabilities using controlled payloads.
...
impl XxePlugin {
    /// Create a new XXE injection plugin
    ...
    fn references(&self) -> Vec<crate::injection::SecurityReference> {
        vec![
            crate::injection::SecurityReference {
                ...
                description: "OWASP Top 10 2021 - Security Misconfiguration (XXE)".to_string(),
            },
            crate::injection::SecurityReference {
                ref_type: "CWE".to_string(),
                id: "CWE-611".to_string(),
                url: "https://cwe.mitre.org/data/definitions/611.html".to_string(),
                description: "Improper Restriction of XML External Entity Reference ('XXE')".to_string(),
            },
        ]
    }
...
impl Plugin for XxePlugin {
    ...
    fn new(config: Self::Config) -> Self {
        Self::new(config).expect("Failed to create XXE injection plugin")
    }
    ...
    async fn execute(&self, request: CapabilityRequest) -> Result<CapabilityResponse> {
        ...
        info!("Starting XXE testing for {}", target_url);
...
impl crate::injection::InjectionPlugin for XxePlugin {
    fn injection_category(&self) -> InjectionCategory {
        InjectionCategory::Xxe
    }
    ...
    fn response_analyzer(&self) -> Box<dyn ResponseAnalyzer> {
        create_response_analyzer(InjectionCategory::Xxe)
    }
}

// Plugin entry point
crate::plugin_entry!(XxePlugin);