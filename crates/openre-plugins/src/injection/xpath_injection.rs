...
//!
//! Detects XPath injection vulnerabilities using safe validation techniques.
...
/// XPath Injection Plugin
pub struct XPathInjectionPlugin {
    base: crate::injection::injection_plugin::BaseInjectionPlugin,
}

impl XPathInjectionPlugin {
    /// Create a new XPath injection plugin
    ...
    fn references(&self) -> Vec<crate::injection::SecurityReference> {
        vec![
            ...
            crate::injection::SecurityReference {
                ...
                id: "CWE-643".to_string(),
                url: "https://cwe.mitre.org/data/definitions/643.html".to_string(),
                description: "Improper Neutralization of Special Elements used in an XPath Query ('XPath Injection')".to_string(),
            },
        ]
    }
...
#[async_trait]
impl Plugin for XPathInjectionPlugin {
    type Config = InjectionPluginConfig;
    
    fn new(config: Self::Config) -> Self {
        Self::new(config).expect("Failed to create XPath injection plugin")
    }
    ...
    async fn execute(&self, request: CapabilityRequest) -> Result<CapabilityResponse> {
        ...
        info!("Starting XPath injection testing for {}", target_url);
...
impl crate::injection::InjectionPlugin for XPathInjectionPlugin {
    fn injection_category(&self) -> InjectionCategory {
        InjectionCategory::XPathInjection
    }
    ...
    fn response_analyzer(&self) -> Box<dyn ResponseAnalyzer> {
        create_response_analyzer(InjectionCategory::XPathInjection)
    }
}

// Plugin entry point
crate::plugin_entry!(XPathInjectionPlugin);