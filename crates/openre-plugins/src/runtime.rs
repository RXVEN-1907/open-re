...
use crate::{capability::*, manifest::*, registry::PluginRegistry};
...
impl NativePluginInstance {
    pub async fn initialize(&self, config: &HashMap<String, serde_json::Value>) -> Result<()> {
        ...
        let init: libloading::Symbol<unsafe extern "C" fn(*const u8, usize) -> i32> =
            unsafe { self.library.get(b"plugin_init") }.map_err(|e| {
                openre_core::Error::Internal(anyhow::anyhow!("Failed to get plugin_init: {}", e))
            })?;
    ...
    pub async fn execute(
        ...
        let execute: libloading::Symbol<
            unsafe extern "C" fn(*const u8, usize, *mut u8, *mut usize) -> i32,
        > = unsafe { self.library.get(b"plugin_execute") }.map_err(|e| {
            openre_core::Error::Internal(anyhow::anyhow!("Failed to get plugin_execute: {}", e))
        })?;
        ...
        unsafe {
            response_buf.set_len(response_len);
        }
        serde_json::from_slice(&response_buf).map_err(|e| openre_core::Error::Serialization(e))
    }

    pub async fn shutdown(&self) -> Result<()> {
        let shutdown: libloading::Symbol<unsafe extern "C" fn() -> i32> = unsafe {
            self.library.get(b"plugin_shutdown")
        }
        .map_err(|e| {
            openre_core::Error::Internal(anyhow::anyhow!("Failed to get plugin_shutdown: {}", e))
        })?;
        let result = unsafe { (shutdown)() };
        if result != 0 {
            return Err(openre_core::Error::Internal(anyhow::anyhow!(
                "Native plugin shutdown failed"
            )));
        }
        Ok(())
    }
        ...
        let plugin = match manifest.build.target {
            BuildTarget::Wasm => {
                let wasm_path = manifest
                    .wasm_path(manifest.path.as_ref().unwrap_or(&PathBuf::from(".")))
            ...
            BuildTarget::Native => {
                let native_path = manifest
                    .native_path(manifest.path.as_ref().unwrap_or(&PathBuf::from(".")))
                    .ok_or_else(|| {
                        openre_core::Error::NotFound("Native library not found".into())
                    })?;
...
impl WasmRuntime {
    pub fn new(config: WasmConfig) -> Result<Self> {
        ...
        add_to_linker_async(&mut linker, |state: &mut PluginState| {
            &mut state.wasi_p1_ctx
        })?;
    ...
    fn add_host_functions(linker: &mut wasmtime::Linker<PluginState>) -> Result<()> {
        ...
        linker.func_wrap(
            "host",
            "read_binary",
            |mut caller: wasmtime::Caller<'_, PluginState>, offset: u64, len: u64| -> u32 {
        ...
        linker.func_wrap(
            ...
            |mut caller: wasmtime::Caller<'_, PluginState>,
             ...
             -> u32 {
                let state = caller.data_mut();
                if state
                    .check_capability(Capability::WriteAnnotations)
                    .is_err()
                {
                    return 1; // Error
                }
        ...
        linker.func_wrap(
            "host",
            "query_database",
            |mut caller: wasmtime::Caller<'_, PluginState>, sql_ptr: u32, sql_len: u32| -> u32 {
                let state = caller.data_mut();
    ...
    fn validate_module(&self, module: &wasmtime::Module) -> Result<()> {
        for import in module.imports() {
            if !self
                ...
                .contains(&import.name().to_string())
            {
...
impl NativeRuntime {
    ...
    pub async fn load_library(&self, path: &PathBuf) -> Result<NativePluginInstance> {
        ...
        let library = unsafe { libloading::Library::new(path) }.map_err(|e| {
            openre_core::Error::Internal(anyhow::anyhow!("Failed to load library: {}", e))
        })?;
    ...
    async fn verify_signature(&self, path: &PathBuf) -> Result<()> {
        ...
        let verified = self
            .trusted_keys
            .iter()
            .any(|key| key.verify(&manifest, &signature).is_ok());

        if !verified {
            return Err(openre_core::Error::Validation(
                "Invalid plugin signature".into(),
            ));
        }
