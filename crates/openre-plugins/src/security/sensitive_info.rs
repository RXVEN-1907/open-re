//! PII, secrets, credentials detection Security Plugin
...
pub fn manifest() -> PluginManifest {
    let metadata = SimplePluginMetadata {
        name: "security-sensitive_info".to_string(),
        version: "0.1.0".to_string(),
        description: "PII, secrets, credentials detection".to_string(),
        ...
        keywords: vec!["security".to_string(), "sensitive-info".to_string()],
    };
...
mod tests {
    ...
    fn test_manifest() {
        let m = manifest();
        assert_eq!(m.name, "security-sensitive_info");
