...
impl ApiRateLimitingPlugin {
    ...
    async fn test_endpoint_rate_limit(&self, url: &str, method: &str, is_auth_endpoint: bool) -> RateLimitTestResult {
                ...
                Ok(response) => {
                    ...
                    if status == 429 {
                        ...
                        for (key, value) in response.headers() {
                            ...
                            if key_str.contains("rate") || key_str.contains("limit") || key_str.contains("retry") {
                                rate_limit_headers.insert(key_str, value.to_str().unwrap_or("").to_string());
                            }
                        }
    ...
    fn analyze_results(&self, result: &RateLimitTestResult) -> Vec<Finding> {
        let mut findings = Vec::new();
        
        // Check 1: No rate limiting detected
        if !result.rate_limited {
            let severity = if result.is_auth_endpoint { Severity::High } else { Severity::Medium };
            findings.push(self.create_finding(
                "Missing Rate Limiting",
                &format!("Endpoint {} {} does not appear to implement rate limiting", result.method, result.url),
                severity,
                Confidence::Medium,
        ...
        if !burst_limited && result.burst_results.len() > 5 {
            findings.push(self.create_finding(
                ...
                &format!("Endpoint {} {} allows burst of {} requests without rate limiting", result.method, result.url, result.burst_results.len()),
                Severity::Medium,
                Confidence::Medium,
                Category::SecurityMisconfiguration,
                result,
        ...
        if result.is_auth_endpoint {
            ...
            if !auth_limited && result.auth_results.len() > 3 {
                findings.push(self.create_finding(
                    ...
                    &format!("Authentication endpoint {} {} allows unlimited login attempts", result.method, result.url),
                    Severity::High,
                    Confidence::High,
        ...
        if result.rate_limited && result.retry_after.is_none() {
            findings.push(self.create_finding(
                ...
                &format!("Endpoint {} {} returns 429 without Retry-After header", result.method, result.url),
                Severity::Low,
                Confidence::High,
                Category::SecurityMisconfiguration,
                result,
...
impl Plugin for ApiRateLimitingPlugin {
    ...
    async fn execute(&self, request: CapabilityRequest) -> Result<CapabilityResponse> {
        ...
        let endpoints = request.input.get("endpoints")
            .and_then(|v| serde_json::from_value::<Vec<EndpointTestConfig>>(v.clone()).ok())
            .unwrap_or_else(|| vec![
                EndpointTestConfig { url: format!("{}/api/login", target_url), method: "POST".to_string(), is_auth_endpoint: true },
                EndpointTestConfig { url: format!("{}/api/register", target_url), method: "POST".to_string(), is_auth_endpoint: true },
                EndpointTestConfig { url: format!("{}/api/users", target_url), method: "GET".to_string(), is_auth_endpoint: false },
                EndpointTestConfig { url: format!("{}/api/data", target_url), method: "GET".to_string(), is_auth_endpoint: false },
            ]);
        
        let endpoints_count = endpoints.len();
        info!("Starting API rate limiting analysis for {} endpoints", endpoints_count);
        
        let mut all_findings = Vec::new();
        for endpoint in endpoints {
            let result = self.test_endpoint_rate_limit(&endpoint.url, &endpoint.method, endpoint.is_auth_endpoint).await;
            let findings = self.analyze_results(&result);
