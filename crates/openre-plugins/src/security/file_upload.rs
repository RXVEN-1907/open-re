...
pub fn manifest() -> PluginManifest {
    let metadata = SimplePluginMetadata {
        name: "security-file_upload".to_string(),
        version: "0.1.0".to_string(),
        description: "Malicious file upload testing".to_string(),
        ...
        keywords: vec!["security".to_string(), "file-upload".to_string()],
    };

    PluginManifest::from_simple(
        metadata,
        vec![Capability::NetworkAccess, Capability::WriteBinary, Capability::CallAi],
        vec![],
    )
}
...
mod tests {
    ...
    fn test_manifest() {
        let m = manifest();
        assert_eq!(m.name, "security-file_upload");
