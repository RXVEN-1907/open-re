...
impl SecurityHeadersPlugin {
    ...
    async fn analyze_headers(&self, base_url: &str) -> HeaderAnalysisResult {
        ...
        result.x_permitted_cross_domain_analysis = self.analyze_x_permitted_cross_domain(&result.headers);
    ...
    fn analyze_csp(&self, headers: &HashMap<String, String>) -> HeaderAnalysis {
        ...
        let csp_value = headers.get("content-security-policy")
        ...
        if let Some(value) = csp_value {
            ...
            if directives.get("default-src").map_or(false, |v| v.contains("'unsafe-inline'") || v.contains("'unsafe-eval'") || v.contains("*")) {
            ...
            if directives.get("script-src").map_or(false, |v| v.contains("'unsafe-inline'") || v.contains("'unsafe-eval'") || v.contains("*")) {
                analysis.issues.push(HeaderIssue {
            ...
            if directives.get("style-src").map_or(false, |v| v.contains("'unsafe-inline'") || v.contains("*")) {
                analysis.issues.push(HeaderIssue {
            ...
            let important_directives = ["default-src", "script-src", "style-src", "img-src", "connect-src", "font-src", "object-src", "frame-src", "base-uri", "form-action"];
            for directive in &important_directives {
                if !directives.contains_key(*directive) {
                    analysis.issues.push(HeaderIssue {
                        ...
                        description: format!("The CSP policy is missing the {} directive", directive),
                        recommendation: format!("Add {} directive to restrict {} resources", directive, directive.replace("-src", "")),
                    });
                    analysis.score -= 5;
                }
            }
            ...
            if !analysis.issues.iter().any(|i| matches!(i.severity, Severity::High | Severity::Critical)) {
                analysis.score += 20;
            }
        } else {
            analysis.issues.push(HeaderIssue {
        ...
        if let Some(value) = hsts_value {
            ...
            if let Some(max_age_str) = directives.get("max-age") {
                ...
                analysis.issues.push(HeaderIssue {
                    ...
                    description: "HSTS header is missing the required max-age directive".to_string(),
                    recommendation: "Add max-age directive with value of at least 31536000".to_string(),
                });
                analysis.score -= 30;
            }
            ...
            if directives.contains_key("includesubdomains") {
                ...
                analysis.issues.push(HeaderIssue {
                    ...
                    description: "HSTS header does not include includeSubDomains directive".to_string(),
            ...
            if directives.contains_key("preload") {
                ...
                analysis.issues.push(HeaderIssue {
                    ...
                    description: "HSTS header does not include preload directive for HSTS preload list".to_string(),
                    recommendation: "Consider adding preload directive and submitting to HSTS preload list".to_string(),
                });
                analysis.score -= 2;
            }
        } else {
        ...
        if let Some(value) = xfo_value {
            ...
            if value_upper == "DENY" {
                ...
                analysis.issues.push(HeaderIssue {
                    ...
                    description: "ALLOW-FROM is deprecated and not supported by all browsers".to_string(),
    ...
    fn analyze_referrer_policy(&self, headers: &HashMap<String, String>) -> HeaderAnalysis {
        ...
        if let Some(value) = referrer_value {
            ...
            let secure_policies = ["no-referrer", "no-referrer-when-downgrade", "strict-origin", "strict-origin-when-cross-origin", "same-origin"];
            let insecure_policies = ["unsafe-url", "origin", "origin-when-cross-origin"];
            
            let has_secure = policies.iter().any(|p| secure_policies.contains(&p.as_str()));
            let has_insecure = policies.iter().any(|p| insecure_policies.contains(&p.as_str()));
            ...
            if has_secure && !has_insecure {
                ...
                analysis.issues.push(HeaderIssue {
                    ...
                    recommendation: "Use secure policies like strict-origin-when-cross-origin or no-referrer".to_string(),
                });
            } else {
                analysis.score = 0;
                analysis.issues.push(HeaderIssue {
                    ...
                    description: format!("Referrer-Policy contains unknown policy values: {}", value),
            ...
            analysis.issues.push(HeaderIssue {
                ...
                description: "The Referrer-Policy header is not present, which may leak referrer information".to_string(),
    ...
    fn analyze_permissions_policy(&self, headers: &HashMap<String, String>) -> HeaderAnalysis {
        ...
        let permissions_value = headers.get("permissions-policy")
        ...
        if let Some(value) = permissions_value {
            ...
            for perm in &dangerous_permissions {
                if value.to_lowercase().contains(&perm.to_lowercase()) && !value.contains(&format!("{} 'none'", perm)) && !value.contains(&format!("{} ()", perm)) {
                    analysis.issues.push(HeaderIssue {
                        issue_type: format!("permissions_{}_enabled", perm.replace("-", "_")),
                        severity: Severity::Low,
                        title: format!("Permissions-Policy Allows {}", perm),
                        description: format!("Permissions-Policy allows access to {} feature", perm),
                        recommendation: format!("Restrict {} permission to 'none' or specific origins if not needed", perm),
                    });
                    analysis.score -= 5;
                }
            }
            ...
            if value.contains("*") {
                analysis.issues.push(HeaderIssue {
                    ...
                    recommendation: "Restrict permissions to specific origins or 'none'".to_string(),
                });
                analysis.score -= 20;
            }
        } else {
    ...
    fn analyze_x_content_type_options(&self, headers: &HashMap<String, String>) -> HeaderAnalysis {
        ...
        if let Some(value) = xcto_value {
            ...
            analysis.issues.push(HeaderIssue {
                ...
                description: "The X-Content-Type-Options header is not present, allowing MIME type sniffing".to_string(),
    ...
    fn analyze_cache_control(&self, headers: &HashMap<String, String>) -> HeaderAnalysis {
        ...
        if let Some(value) = cache_value {
            ...
            let directives: Vec<String> = value.split(',').map(|s| s.trim().to_lowercase()).collect();
                    ...
                    if let Ok(max_age) = directive[8..].parse::<i64>() {
                        if max_age > 31536000 { // 1 year
                            analysis.issues.push(HeaderIssue {
                                ...
                                description: format!("Cache-Control max-age is {} seconds ({} days)", max_age, max_age / 86400),
                                recommendation: "Consider shorter max-age for dynamic content".to_string(),
                            });
                            analysis.score -= 5;
                        }
                    }
        ...
        if let Some(value) = xss_value {
            ...
            if value == "1; mode=block" {
                ...
                analysis.issues.push(HeaderIssue {
                    ...
                    description: "X-XSS-Protection is a legacy header, modern browsers rely on CSP instead".to_string(),
                    recommendation: "Implement Content-Security-Policy instead of relying on X-XSS-Protection".to_string(),
                });
            } else if value == "0" {
                analysis.score = 0;
                analysis.issues.push(HeaderIssue {
                    ...
                    recommendation: "Enable X-XSS-Protection: 1; mode=block or better, implement CSP".to_string(),
                });
            } else {
                analysis.score = 40;
                analysis.issues.push(HeaderIssue {
                    ...
                    recommendation: "Use X-XSS-Protection: 1; mode=block or implement CSP".to_string(),
                });
            }
        } else {
            analysis.issues.push(HeaderIssue {
                ...
                description: "X-XSS-Protection header is not present (legacy header, CSP is preferred)".to_string(),
                recommendation: "Implement Content-Security-Policy for modern XSS protection".to_string(),
            });
            analysis.score = 0;
        }
    ...
    fn analyze_x_permitted_cross_domain(&self, headers: &HashMap<String, String>) -> HeaderAnalysis {
        let mut analysis = HeaderAnalysis {
        ...
        if let Some(value) = xpcd_value {
            ...
            if value_lower == "none" {
                ...
                analysis.issues.push(HeaderIssue {
                    ...
                    recommendation: "Use 'none' for maximum security unless cross-domain policies are needed".to_string(),
                });
            } else if value_lower == "by-content-type" || value_lower == "by-ftp-filename" || value_lower == "all" {
                analysis.score = 20;
                analysis.issues.push(HeaderIssue {
                    issue_type: "xpcd_permissive".to_string(),
                    severity: Severity::Medium,
                    title: "Permissive X-Permitted-Cross-Domain-Policies".to_string(),
                    description: format!("X-Permitted-Cross-Domain-Policies allows cross-domain access: {}", value),
                    recommendation: "Set to 'none' unless Flash/legacy cross-domain access is required".to_string(),
                });
            } else {
                analysis.score = 0;
                analysis.issues.push(HeaderIssue {
                    ...
                    description: format!("X-Permitted-Cross-Domain-Policies has unknown value: {}", value),
                    recommendation: "Use 'none', 'master-only', 'by-content-type', 'by-ftp-filename', or 'all'".to_string(),
                });
            }
        } else {
            analysis.issues.push(HeaderIssue {
                ...
                description: "Header not present (mainly relevant for legacy Flash content)".to_string(),
                recommendation: "Add X-Permitted-Cross-Domain-Policies: none for defense in depth".to_string(),
            });
            analysis.score = 0;
        }
    ...
    fn analyze_cross_origin_headers(&self, headers: &HashMap<String, String>) -> HeaderAnalysis {
        ...
        if let Some(value) = coop {
            ...
            analysis.issues.push(HeaderIssue {
                ...
                description: "COOP header not present, cross-origin isolation not enabled".to_string(),
                recommendation: "Add Cross-Origin-Opener-Policy: same-origin for cross-origin isolation".to_string(),
            });
        }
        ...
        if let Some(value) = coep {
            ...
            if value == "require-corp" {
                ...
                analysis.issues.push(HeaderIssue {
                    ...
                    recommendation: "Use require-corp or credentialless for cross-origin isolation".to_string(),
                });
            }
        } else {
            analysis.issues.push(HeaderIssue {
                ...
                description: "COEP header not present, cross-origin isolation not enabled".to_string(),
                recommendation: "Add Cross-Origin-Embedder-Policy: require-corp for cross-origin isolation".to_string(),
            });
        }
        ...
        if let Some(value) = corp {
            ...
            if value == "same-origin" {
                ...
                analysis.issues.push(HeaderIssue {
                    ...
                    description: "Cross-Origin-Resource-Policy allows any origin to load this resource".to_string(),
                    recommendation: "Use same-origin or same-site unless cross-origin loading is required".to_string(),
                });
            }
        } else {
            analysis.issues.push(HeaderIssue {
        ...
        analysis.value = Some(format!("COOP: {:?}, COEP: {:?}, CORP: {:?}", coop_value, coep_value, corp_value));
...
impl Plugin for SecurityHeadersPlugin {
    ...
    async fn execute(&self, request: CapabilityRequest) -> crate::sdk::Result<CapabilityResponse> {
        ...
        let high_severity_issues = all_analyses.iter()
            .flat_map(|a| &a.issues)
        ...
        let mut summary_finding = Finding::new(
            "Security Headers Analysis Summary".to_string(),
            format!(
            ...
            if high_severity_issues > 0 { Severity::High } else if total_issues > 0 { Severity::Medium } else { Severity::Info },
            Confidence::High,
        ...
        for header_analysis in all_analyses {
            for issue in &header_analysis.issues {
                let mut finding = Finding::new(
                    format!("Header Issue: {} - {}", header_analysis.header_name, issue.title),
