...
use crate::security::{
    ...
    HttpResponse, is_auth_page, detect_sso_providers, detect_mfa_indicators,
};
...
impl AuthDiscoveryPlugin {
    ...
    fn detect_login_form(&self, body: &str) -> bool {
        ...
        let submit_indicators = ["type=\"submit\"", "type='submit'", "button", "input type=\"button\""];
    ...
    fn extract_form_fields(&self, body: &str) -> Vec<FormField> {
        ...
        let input_regex = regex::Regex::new(r#"<input[^>]*name=["']([^"']+)["'][^>]*type=["']([^"']+)["']"#).unwrap();
            ...
            if let Some(info) = self.check_endpoint(&endpoint).await {
                if info.is_auth_page {
                    ...
                    let mut finding = Finding::new(
                        format!("Authentication Endpoint Discovered: {}", endpoint),
                        format!(
                            ...
                            info.login_form, info.registration_form, info.password_reset_form,
                            info.mfa_indicators, info.sso_providers, info.oauth_indicators
                        ),
                        Severity::Info,
                        Confidence::High,
                    ...
                    finding = finding.with_evidence(Evidence {
                        ...
                        description: format!("Authentication endpoint response (status: {})", info.status),
                    ...
                    // Add tags
                    if info.login_form { finding = finding.with_tag("login_form".to_string()); }
                    if info.registration_form { finding = finding.with_tag("registration_form".to_string()); }
                    if info.password_reset_form { finding = finding.with_tag("password_reset".to_string()); }
                    for mfa in &info.mfa_indicators { finding = finding.with_tag(format!("mfa:{}", mfa.to_lowercase().replace(' ', "_"))); }
                    for sso in &info.sso_providers { finding = finding.with_tag(format!("sso:{}", sso.to_lowercase().replace(' ', "_"))); }
        ...
        for endpoint in &sso_endpoints {
            ...
            if let Some(info) = self.check_endpoint(&url).await {
                if info.status == 200 || info.status == 302 {
                    let mut finding = Finding::new(
                        format!("SSO/OAuth Endpoint Discovered: {}", endpoint),
                        format!("Discovered SSO/OAuth endpoint at {} with status {}", info.url, info.status),
                        Severity::Info,
                        Confidence::Medium,
                        Category::BrokenAuthentication,
                        info.url.clone(),
