//! XSS detection (reflected, stored, DOM) Security Plugin
...
pub fn manifest() -> PluginManifest {
    let metadata = SimplePluginMetadata {
        name: "security-xss_analysis".to_string(),
        version: "0.1.0".to_string(),
        description: "XSS detection (reflected, stored, DOM)".to_string(),
        ...
        keywords: vec!["security".to_string(), "xss-analysis".to_string()],
    };
...
mod tests {
    ...
    fn test_manifest() {
        let m = manifest();
        assert_eq!(m.name, "security-xss_analysis");
