...
impl CorsAnalysisPlugin {
    ...
    async fn analyze_cors(&self, base_url: &str) -> CorsAnalysisResult {
        ...
        let cred_result = self
            .test_with_credentials(base_url, "https://evil.com")
            .await;
    ...
    fn analyze_origin_response(&self, result: &mut OriginTestResult, test_origin: &str) {
        ...
        if let Some(acao) = &result.acao_header {
            if acao == test_origin
                && test_origin != "null"
                && !test_origin.starts_with("http://localhost")
                && !test_origin.starts_with("https://localhost")
            {
                ...
                if methods.iter().any(|m| m == method) {
                    result.issues.push(CorsIssue {
                        ...
                        recommendation: format!(
                            "Only allow necessary methods. Consider removing {} unless required",
                            method
                        ),
                    });
                }
        ...
        if result.success {
            if let Some(acao) = &result.acao_header {
                if acao == "*" {
                    result.issues.push(CorsIssue {
                        ...
                        recommendation:
                            "Restrict preflight Access-Control-Allow-Origin to specific origins"
                                .to_string(),
                    });
                } else if acao == origin {
                    result.issues.push(CorsIssue {
                        ...
                        recommendation:
                            "Validate Origin header against whitelist in preflight response"
                                .to_string(),
                    });
                }
            }
            ...
            if let Some(acam) = &result.acam_header {
                let methods: Vec<String> =
                    acam.split(',').map(|s| s.trim().to_uppercase()).collect();
                if methods.contains(&"PUT".to_string())
                    || methods.contains(&"DELETE".to_string())
                    || methods.contains(&"PATCH".to_string())
                {
                    result.issues.push(CorsIssue {
                        ...
                        description: format!(
                            "Preflight Access-Control-Allow-Methods includes unsafe methods: {}",
                            acam
                        ),
                        recommendation: "Only allow necessary methods in preflight response"
                            .to_string(),
                    });
                }
            }
    ...
    async fn test_with_credentials(&self, url: &str, origin: &str) -> CredentialsTestResult {
            ...
            .await
        {
            ...
            Err(e) => {
                debug!(
                    "Credentials test failed for {} with origin {}: {}",
                    url, origin, e
                );
                return result;
            }
        ...
        if result.acac_header.as_deref() == Some("true") {
            ...
            if result.acao_header.as_deref() == Some("*") {
                ...
                result.issues.push(CorsIssue {
                    ...
                    description: "Server allows credentials and reflects the Origin header"
                        .to_string(),
                    recommendation: "Validate Origin against whitelist before allowing credentials"
                        .to_string(),
                });
            }
        }
    ...
    async fn test_method(&self, url: &str, method: &str, origin: &str) -> MethodTestResult {
            ...
            .await
        {
            ...
            Err(e) => {
                debug!(
                    "Method test failed for {} {} with origin {}: {}",
                    method, url, origin, e
                );
                return result;
            }
                ...
                || method == "CONNECT")
        {
            result.issues.push(CorsIssue {
                ...
                description: format!(
                    "The {} method is allowed for cross-origin requests from {}",
                    method, origin
                ),
                recommendation: format!(
                    "Restrict {} method to same-origin only unless absolutely necessary",
                    method
                ),
            });
        }
...
impl Plugin for CorsAnalysisPlugin {
    ...
    async fn execute(&self, request: CapabilityRequest) -> crate::sdk::Result<CapabilityResponse> {
        ...
        let severity = if critical_severity > 0 {
            ...
            Severity::High
        } else if all_issues.len() > 0 {
            Severity::Medium
        } else {
            Severity::Info
        };
        let mut summary_finding = Finding::new(FindingConfig {
            title: "CORS Configuration Analysis Summary".to_string(),
            description: format!(
            ...
            plugin_source: "cors_analysis".to_string(),
        ...
        for issue in &all_issues {
            let mut finding = Finding::new(FindingConfig {
                title: format!("CORS Issue: {}", issue.title),
                description: format!(
                    "{}\n\nRecommendation: {}",
