...
impl InformationDisclosurePlugin {
    ...
    async fn analyze_information_disclosure(&self, base_url: &str) -> InfoDisclosureResult {
        ...
        for endpoint in debug_endpoints {
            ...
            if let Some(resp) = response {
                if resp.status == 200 || resp.status == 403 || resp.status == 500 {
                    result.debug_endpoint_analyses.push(self.analyze_response(&resp, &format!("debug:{}", endpoint)));
                }
            }
        }
        ...
        if let Some(main) = &result.main_page_analysis {
            result.technology_analysis = self.analyze_technology_disclosure(&main.headers, &main.body_snippet);
        }
        ...
        for file in sensitive_files {
            ...
            if let Some(resp) = response {
                if resp.status == 200 {
                    result.sensitive_file_analyses.push(self.analyze_response(&resp, &format!("sensitive_file:{}", file)));
                }
            }
        }
        ...
        for endpoint in api_doc_endpoints {
            ...
            if let Some(resp) = response {
                if resp.status == 200 {
                    result.api_doc_analyses.push(self.analyze_response(&resp, &format!("api_doc:{}", endpoint)));
                }
            }
        }
    ...
    fn check_header_disclosure(&self, analysis: &mut ResponseAnalysis) {
        ...
        if let Some(runtime) = analysis.headers.get("x-runtime") {
            analysis.issues.push(InfoDisclosureIssue {
                ...
                description: format!("X-Runtime header reveals request processing time: {}", runtime),
        ...
        if let Some(via) = analysis.headers.get("via") {
            analysis.issues.push(InfoDisclosureIssue {
                ...
                evidence: via.clone(),
                recommendation: "Configure proxies to not forward Via header or minimize information".to_string(),
            });
        }
        
        // X-Forwarded-* headers
        for header in ["x-forwarded-for", "x-forwarded-host", "x-forwarded-proto", "x-forwarded-server"] {
            if let Some(value) = analysis.headers.get(header) {
                analysis.issues.push(InfoDisclosureIssue {
                    issue_type: format!("{}_exposure", header.replace("-", "_")),
                    severity: Severity::Info,
                    ...
                    evidence: value.clone(),
                    recommendation: "Ensure these headers are only added by trusted proxies".to_string(),
                });
            }
        }
    ...
    fn check_body_disclosure(&self, analysis: &mut ResponseAnalysis) {
        ...
        for (pattern, desc) in &version_patterns {
            ...
            if regex.is_match(&body_lower) {
                analysis.issues.push(InfoDisclosureIssue {
                    ...
                    description: format!("Response body contains a {} pattern", desc.to_lowercase()),
                    evidence: self.extract_match(body, pattern).unwrap_or_default(),
                    recommendation: "Remove version information from client-facing responses".to_string(),
                });
                break;
            }
        }
        ...
        for (marker, name) in &framework_markers {
            if body_lower.contains(marker) {
                analysis.issues.push(InfoDisclosureIssue {
                    ...
                    recommendation: "Consider removing framework identifiers from client-facing responses".to_string(),
                });
            }
        }
        ...
        if body_lower.contains("<?php") || body_lower.contains("<%@") || body_lower.contains("<%=") ||
           body_lower.contains("<?=") || body_lower.contains("jsp:") {
            analysis.issues.push(InfoDisclosureIssue {
                ...
                recommendation: "Ensure server-side code is properly executed, not served as plain text".to_string(),
            });
        }
        ...
        let backup_patterns = [
            ...
            r#"\.swp"#, r#"\.swo"#, r#"~$"#, r#"\.tmp"#, r#"\.temp"#,
            r#"config\.bak"#, r#"\.env\.bak"#, r#"settings\.bak"#,
        ];
        ...
        for pattern in &backup_patterns {
            ...
            if regex.is_match(&body_lower) {
                analysis.issues.push(InfoDisclosureIssue {
                    ...
                    description: format!("Response contains reference to backup or temporary file pattern: {}", pattern),
                    evidence: pattern.to_string(),
                    recommendation: "Ensure backup and temporary files are not accessible via web server".to_string(),
                });
                break;
            }
        }
        ...
        for pattern in &comment_patterns {
            ...
            if regex.is_match(&body_lower) {
                analysis.issues.push(InfoDisclosureIssue {
                    ...
                    description: "HTML/JS comments contain potentially sensitive keywords".to_string(),
                    evidence: self.extract_match(body, pattern).unwrap_or_default(),
    ...
    fn analyze_server_header(&self, headers: &HashMap<String, String>) -> ServerHeaderAnalysis {
        ...
        if let Some(server) = &analysis.server_header {
            ...
            if server.to_lowercase().contains("ubuntu") || server.to_lowercase().contains("debian") ||
               server.to_lowercase().contains("centos") || server.to_lowercase().contains("red hat") {
                analysis.issues.push(InfoDisclosureIssue {
                    ...
                    evidence: server.clone(),
                    recommendation: "Configure server to minimize Server header information".to_string(),
                });
            }
        }
    ...
    fn analyze_technology_disclosure(&self, headers: &HashMap<String, String>, body: &str) -> TechnologyAnalysis {
        ...
        for tech in &analysis.detected_technologies {
            analysis.issues.push(InfoDisclosureIssue {
                issue_type: format!("tech_{}", tech.name.to_lowercase().replace(" ", "_").replace(".", "_")),
                severity: Severity::Info,
                ...
                recommendation: "Consider hiding technology stack information in production".to_string(),
            });
        }
    ...
    fn extract_version_from_body(&self, body: &str, marker: &str) -> Option<String> {
        ...
        let pattern = format!(r"{}[^a-zA-Z0-9]*([\d]+\.[\d]+(\.[\d]+)?)", regex::escape(marker));
        let regex = Regex::new(&pattern).ok()?;
        regex.captures(body).and_then(|c| c.get(1)).map(|m| m.as_str().to_string())
    }
    
    fn get_debug_endpoints(&self) -> Vec<&'static str> {
        vec![
            "/debug", "/debug/", "/debug/default/view",
            "/actuator", "/actuator/", "/actuator/health", "/actuator/info", "/actuator/env", "/actuator/metrics",
            "/health", "/health/", "/healthz", "/ready", "/live",
            "/metrics", "/metrics/", "/prometheus", "/prometheus/metrics",
            "/status", "/status/", "/server-status", "/server-status/",
            "/admin", "/admin/", "/adminer", "/phpmyadmin", "/pma",
            "/swagger", "/swagger/", "/swagger-ui", "/swagger-ui/",
            "/api-docs", "/api-docs/", "/openapi", "/openapi.json",
            "/graphql", "/graphql/", "/playground", "/playground/",
            "/console", "/console/", "/h2-console", "/h2-console/",
            "/_profiler", "/_profiler/", "/_wdt", "/_wdt/",
            "/trace", "/trace/", "/traces", "/traces/",
            "/dump", "/dump/", "/heapdump", "/heapdump/",
            "/env", "/env/", "/config", "/config/",
            "/info", "/info/", "/system/info", "/system/info/",
        ]
    }
...
impl Plugin for InformationDisclosurePlugin {
    ...
    async fn execute(&self, request: CapabilityRequest) -> crate::sdk::Result<CapabilityResponse> {
        ...
        info!("Starting information disclosure analysis for {}", target_url);
        ...
        let high_severity = all_issues.iter().filter(|i| matches!(i.severity, Severity::High)).count();
        let medium_severity = all_issues.iter().filter(|i| matches!(i.severity, Severity::Medium)).count();
        
        // Summary finding
        let mut summary_finding = Finding::new(
            "Information Disclosure Analysis Summary".to_string(),
            format!(
            ...
            if critical_severity > 0 { Severity::Critical } else if high_severity > 0 { Severity::High } else if medium_severity > 0 { Severity::Medium } else { Severity::Info },
            Confidence::High,
        ...
        for issue in &all_issues {
            let mut finding = Finding::new(
                format!("Information Disclosure: {}", issue.title),
                format!("{}\n\nEvidence: {}\n\nRecommendation: {}", issue.description, issue.evidence, issue.recommendation),
                issue.severity,
                Confidence::High,
                Category::InformationDisclosure,
                target_url.to_string(),
    ...
    fn references(&self) -> Vec<SecurityReference> {
        ...
        refs.extend(vec![
            ...
            SecurityReference {
                ...
                description: "Exposure of System Data to an Unauthorized Control Sphere".to_string(),
            },
            SecurityReference {
                ref_type: "CWE".to_string(),
