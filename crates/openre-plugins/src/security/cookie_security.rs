...
use crate::security::{
    extract_cookies, standard_references, CookieInfo, HttpResponse, SecurityPlugin,
    SecurityPluginConfig, SecurityReference,
};
...
impl CookieSecurityPlugin {
    ...
    fn analyze_single_cookie(&self, cookie: &CookieInfo, base_url: &str) -> CookieAnalysis {
        ...
        if !cookie.secure {
            analysis.issues.push(CookieIssue {
                ...
                recommendation:
                    "Set the Secure flag to ensure cookie is only transmitted over HTTPS"
                        .to_string(),
        ...
        if !cookie.http_only {
            analysis.issues.push(CookieIssue {
                ...
                impact: "Cookie accessible to client-side JavaScript, increasing XSS risk"
                    .to_string(),
        ...
        match cookie.same_site.as_deref() {
            None => {
                analysis.issues.push(CookieIssue {
                    ...
                    impact: "Cookie sent with cross-site requests, enabling CSRF attacks"
                        .to_string(),
                    recommendation: "Set SameSite to Lax or Strict to prevent CSRF attacks"
                        .to_string(),
                    references: vec![SecurityReference {
                        ref_type: "CWE".to_string(),
            ...
            Some("none") => {
                if !cookie.secure {
                    analysis.issues.push(CookieIssue {
                        ...
                        description: format!(
                            "Cookie '{}' has SameSite=None but is missing Secure flag",
                            cookie.name
                        ),
                        impact: "Cookie will be rejected by modern browsers".to_string(),
                        recommendation: "Either add Secure flag or change SameSite to Lax/Strict"
                            .to_string(),
                        references: vec![],
                    });
                    analysis.score -= 30;
                    ...
                    analysis.issues.push(CookieIssue {
                        ...
                        description: format!(
                            "Cookie '{}' has SameSite=None, allowing cross-site requests",
                            cookie.name
                        ),
                        impact: "Cookie sent with all cross-site requests".to_string(),
                        recommendation:
                            "Only use SameSite=None with Secure for legitimate cross-site cookies"
                                .to_string(),
                        references: vec![],
                    });
                    analysis.score -= 5;
            ...
            Some(other) => {
                analysis.issues.push(CookieIssue {
                    ...
                    description: format!(
                        "Cookie '{}' has invalid SameSite value: {}",
                        cookie.name, other
                    ),
        ...
        if let Some(domain) = &cookie.domain {
            ...
            let target_domain = url::Url::parse(base_url)
                .ok()
            ...
            if let Some(target) = target_domain {
                if !domain.contains(&target) && !target.contains(domain.trim_start_matches('.')) {
                    analysis.issues.push(CookieIssue {
                        ...
                        description: format!(
                            "Cookie '{}' domain '{}' doesn't match target domain '{}'",
                            cookie.name, domain, target
                        ),
                        impact:
                            "Cookie may be set by a different domain (potential cookie tossing)"
                                .to_string(),
                        recommendation: "Ensure cookies are only set for the intended domain"
                            .to_string(),
                        references: vec![],
                    });
                    analysis.score -= 15;
        ...
        if has_expiration {
            if let Some(max_age) = cookie.max_age {
                if max_age > 31536000 {
                    ...
                    analysis.issues.push(CookieIssue {
                        ...
                        description: format!(
                            "Cookie '{}' expires in {} seconds ({} days)",
                            cookie.name,
                            max_age,
                            max_age / 86400
                        ),
                        ...
                        recommendation:
                            "Use shorter expiration times for session cookies (e.g., 24 hours)"
                                .to_string(),
                        references: vec![SecurityReference {
                            ref_type: "CWE".to_string(),
                ...
                if let Ok(expires_dt) = DateTime::parse_from_rfc2822(expires) {
                    ...
                    if diff.num_days() > 365 {
                        analysis.issues.push(CookieIssue {
                            ...
                            impact: "Long-lived cookies increase risk if compromised".to_string(),
                            recommendation: "Use shorter expiration times for session cookies"
                                .to_string(),
                            references: vec![],
                        });
                        analysis.score -= 10;
            ...
            analysis.issues.push(CookieIssue {
                ...
                description: format!(
                    "Cookie '{}' is a session cookie (expires when browser closes)",
                    cookie.name
                ),
                impact: "Session cookies are cleared when browser closes - good for security"
                    .to_string(),
        ...
        if self.is_weak_cookie_value(&cookie.value) {
            analysis.issues.push(CookieIssue {
                ...
                description: format!(
                    "Cookie '{}' has a potentially weak or predictable value",
                    cookie.name
                ),
                impact: "Session ID could be guessed or brute-forced".to_string(),
                recommendation:
                    "Use cryptographically secure random values (at least 128 bits of entropy)"
                        .to_string(),
                references: vec![SecurityReference {
                    ref_type: "CWE".to_string(),
        ...
        if cookie.name.starts_with("__Secure-") && !cookie.secure {
            analysis.issues.push(CookieIssue {
                ...
                description: format!(
                    "Cookie '{}' has __Secure- prefix but is missing Secure flag",
                    cookie.name
                ),
        ...
        if cookie.name.starts_with("__Host-") {
            if !cookie.secure {
                analysis.issues.push(CookieIssue {
                    ...
                    description: format!(
                        "Cookie '{}' has __Host- prefix but is missing Secure flag",
                        cookie.name
                    ),
                    impact: "Browser will reject this cookie".to_string(),
            ...
            if cookie.domain.is_some() {
                analysis.issues.push(CookieIssue {
                    ...
                    description: format!(
                        "Cookie '{}' has __Host- prefix but has a Domain attribute",
                        cookie.name
                    ),
                    impact: "Browser will reject this cookie".to_string(),
                    recommendation: "Remove Domain attribute from cookies with __Host- prefix"
                        .to_string(),
                    references: vec![],
                });
                analysis.score -= 25;
            ...
            if cookie.path.as_deref() != Some("/") {
                analysis.issues.push(CookieIssue {
                    ...
                    description: format!(
                        "Cookie '{}' has __Host- prefix but path is not '/'",
                        cookie.name
                    ),
                    impact: "Browser will reject this cookie".to_string(),
    ...
    fn is_weak_cookie_value(&self, value: &str) -> bool {
        ...
        // Sequential patterns
        if value_lower.contains("123456")
            || value_lower.contains("abcdef")
            || value_lower.contains("qwerty")
        {
            return true;
        }
...
impl Plugin for CookieSecurityPlugin {
    ...
    async fn execute(&self, request: CapabilityRequest) -> crate::sdk::Result<CapabilityResponse> {
        ...
        let cookies_with_issues = analysis
            .analyses
            .iter()
            .filter(|a| !a.issues.is_empty())
            .count();
        let high_severity_issues = analysis
            .analyses
            .iter()
            .flat_map(|a| &a.issues)
        ...
        if total_cookies > 0 {
            let severity = if high_severity_issues > 0 {
                Severity::High
            } else if cookies_with_issues > 0 {
                Severity::Medium
            } else {
                Severity::Info
            };
            let mut finding = Finding::new(FindingConfig {
                title: "Cookie Security Analysis Summary".to_string(),
                description: format!(
                ...
                plugin_source: "cookie_security".to_string(),
        ...
        for cookie_analysis in &analysis.analyses {
            for issue in &cookie_analysis.issues {
                let mut finding = Finding::new(FindingConfig {
                    title: format!(
                        "Cookie Issue: {} - {}",
                        cookie_analysis.cookie.name, issue.title
                    ),
                    description: format!(
                        "{}\n\nImpact: {}\n\nRecommendation: {}",
