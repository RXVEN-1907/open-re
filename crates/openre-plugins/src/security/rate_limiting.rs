...
impl RateLimitingPlugin {
    ...
    async fn discover_test_endpoints(&self, base_url: &str) -> Vec<TestEndpoint> {
        ...
        let common_paths = [
            "/", "/home", "/index", "/api", "/api/v1", "/api/v2", "/health", "/status", "/ping",
            "/ready",
        ];
    ...
    fn estimate_rate_limit(&self, test: &EndpointRateLimitTest) -> Option<u32> {
        if let Some(first_429) = test
            .response_times
            .len()
            .checked_sub(test.requests_rate_limited as usize)
        {
    ...
    async fn sustained_rate_test(
        &self,
        base_url: &str,
        endpoint: &TestEndpoint,
        mut test: EndpointRateLimitTest,
    ) -> EndpointRateLimitTest {
    ...
    fn analyze_rate_limit_results(&self, test: &mut EndpointRateLimitTest) {
        ...
        if test.rate_limit_detected {
            if let Some(limit) = test.estimated_limit {
                if limit > 1000 {
                    test.issues.push(RateLimitIssue {
                        ...
                        description: format!(
                            "Estimated rate limit is {} requests, which may be too high",
                            limit
                        ),
                        recommendation:
                            "Consider lowering rate limits, especially for authentication endpoints"
                                .to_string(),
                    });
                    ...
                    test.issues.push(RateLimitIssue {
                        ...
                        description: format!(
                            "Authentication endpoint has low rate limit ({} requests)",
                            limit
                        ),
                        recommendation:
                            "This is good for security. Ensure legitimate users aren't blocked"
                                .to_string(),
                    });
                }
            }
            ...
            if !test.rate_limit_headers.contains_key("retry-after")
                ...
                && !test
                    .rate_limit_headers
                    .contains_key("x-ratelimit-reset-after")
            {
                test.issues.push(RateLimitIssue {
        ...
        if test.requests_rate_limited > 0
            && test.requests_succeeded > test.requests_rate_limited * 2
        {
            test.issues.push(RateLimitIssue {
                ...
                recommendation: "Ensure rate limiting is consistently applied across all requests"
                    .to_string(),
            });
        }
    }
...
impl Plugin for RateLimitingPlugin {
    ...
    async fn execute(&self, request: CapabilityRequest) -> crate::sdk::Result<CapabilityResponse> {
        ...
        let critical_severity = all_issues
            ...
            .count();
        let no_rate_limit_count = all_issues
            .iter()
            .filter(|i| i.issue_type == "no_rate_limiting")
            .count();
        ...
        let severity = if critical_severity > 0 {
            ...
            Severity::High
        } else if no_rate_limit_count > 0 {
            Severity::Medium
        } else {
            Severity::Info
        };
        let mut summary_finding = Finding::new(FindingConfig {
            title: "Rate Limiting Analysis Summary".to_string(),
            description: format!(
            ...
            severity,
            confidence: Confidence::Medium, // Rate limiting tests are inherently probabilistic
            category: Category::SecurityMisconfiguration,
            target: target_url.to_string(),
            target_type: "web_application".to_string(),
            plugin_source: "rate_limiting".to_string(),
        ...
        for issue in &all_issues {
            let mut finding = Finding::new(FindingConfig {
                title: format!("Rate Limiting Issue: {}", issue.title),
                description: format!(
                    "{}\n\nRecommendation: {}",
