...
impl GraphqlPlugin {
        ...
        for path in common_paths {
                ...
                .await 
            {
                ...
                if status.is_success() || status.as_u16() == 400 || status.as_u16() == 401 || status.as_u16() == 403 {
                    ...
                    let introspection_enabled = body.contains("__schema") || body.contains("queryType");
    ...
    async fn test_endpoint(&self, endpoint: &GraphqlEndpoint) -> Vec<Finding> {
        ...
        if endpoint.introspection_enabled {
            findings.push(self.create_finding(
                ...
                &format!("GraphQL endpoint {} has introspection enabled, exposing full schema", endpoint.url),
                Severity::Medium,
                Confidence::High,
        ...
        if !endpoint.requires_auth {
            findings.push(self.create_finding(
                ...
                &format!("GraphQL endpoint {} does not require authentication", endpoint.url),
                Severity::High,
                Confidence::High,
        ...
        if endpoint.introspection_enabled {
            if let Some(schema_info) = self.fetch_full_schema(&endpoint.url).await {
                ...
                let sensitive_types = ["password", "secret", "token", "key", "credential", "auth", "internal", "admin", "private"];
                for sensitive in &sensitive_types {
                    if schema_info.type_names.iter().any(|name| name.to_lowercase().contains(sensitive)) {
                        findings.push(self.create_finding(
                            "Potentially Sensitive Type in GraphQL Schema",
                            &format!("GraphQL schema contains type with sensitive name: {}", sensitive),
                            Severity::Medium,
                            Confidence::Medium,
                            Category::InformationDisclosure,
                            endpoint,
                            vec!["sensitive-type".to_string(), "schema-exposure".to_string()],
                            vec!["Review if sensitive types should be exposed via GraphQL".to_string()],
                        ));
                    }
                }
            }
                ...
                .await 
            {
                if resp.status().is_success() {
                    findings.push(self.create_finding(
                        ...
                        &format!("GraphQL endpoint {} accepts complex queries without cost analysis", endpoint.url),
                        Severity::Low,
                        Confidence::Medium,
                        Category::SecurityMisconfiguration,
                        endpoint,
            ...
            .await 
        {
            if resp.status().is_success() {
                ...
                if let Ok(json) = serde_json::from_str::<serde_json::Value>(&body) {
                    if let Some(types) = json.get("data").and_then(|d| d.get("__schema")).and_then(|s| s.get("types")).and_then(|t| t.as_array()) {
    ...
    async fn discover_mutations(&self, url: &str) -> Option<Vec<String>> {
            ...
            .await 
        {
            if resp.status().is_success() {
                let body = resp.text().await.ok()?;
                if let Ok(json) = serde_json::from_str::<serde_json::Value>(&body) {
                    if let Some(fields) = json.get("data")
                        .and_then(|d| d.get("__schema"))
                        .and_then(|s| s.get("mutationType"))
                        .and_then(|m| m.get("fields"))
                        .and_then(|f| f.as_array()) 
                    {
                        let mutations: Vec<String> = fields.iter()
