...
impl SessionManagementPlugin {
    ...
    async fn analyze_session(&self, base_url: &str) -> SessionAnalysisResult {
        ...
        let fixation_response = self
            .make_request(base_url, Some(&result.initial_cookies))
            .await;
        if let Some(resp) = fixation_response {
            result.fixation_test_cookies = extract_cookies(&resp.headers, base_url);
            result.session_fixation_possible =
                self.check_session_fixation(&result.initial_cookies, &result.fixation_test_cookies);
        }
    ...
    fn identify_session_cookies(&self, cookies: &[CookieInfo]) -> Vec<SessionCookieInfo> {
        ...
        for cookie in cookies {
            ...
            let looks_like_session =
                cookie.http_only && cookie.secure && cookie.path.as_deref() == Some("/");

            if is_session || looks_like_session {
                session_cookies.push(SessionCookieInfo {
                    cookie: cookie.clone(),
                    confidence: if is_session {
                        Confidence::High
                    } else {
                        Confidence::Medium
                    },
    ...
    fn get_session_reasons(&self, name_lower: &str, cookie: &CookieInfo) -> Vec<String> {
        ...
        if cookie.same_site.as_deref() == Some("lax")
            || cookie.same_site.as_deref() == Some("strict")
        {
            reasons.push(format!(
                "SameSite: {}",
                cookie.same_site.as_deref().unwrap()
            ));
        }
        ...
        for cookie in cookies {
            ...
            match cookie.same_site.as_deref() {
                ...
                Some(other) => {
                    issues.push(CookieSecurityIssue {
                        ...
                        recommendation: "Use Lax, Strict, or None for SameSite attribute"
                            .to_string(),
                    });
                }
            }
            ...
            if let Some(domain) = &cookie.domain {
                if domain.starts_with('.') {
                    issues.push(CookieSecurityIssue {
                        ...
                        recommendation: "Restrict cookie domain to the minimum necessary"
                            .to_string(),
                    });
                }
            }
            ...
            if self.is_weak_cookie_value(&cookie.value) {
                issues.push(CookieSecurityIssue {
                    ...
                    description: format!(
                        "Cookie '{}' has a potentially weak/predictable value",
                        cookie.name
                    ),
                    recommendation:
    ...
    async fn execute(&self, request: CapabilityRequest) -> crate::sdk::Result<CapabilityResponse> {
        ...
        for session_cookie in &analysis.session_cookies {
            let mut finding = Finding::new(FindingConfig {
                title: format!("Session Cookie Identified: {}", session_cookie.cookie.name),
                description: format!(
        ...
        if analysis.session_fixation_possible {
            let mut finding = Finding::new(FindingConfig {
                title: "Potential Session Fixation Vulnerability".to_string(),
                description: "The application appears to accept a pre-existing session ID without regenerating it after authentication. This could allow an attacker to fixate a user's session ID.".to_string(),
                severity: Severity::High,
            ...
            finding = finding.with_cvss(
                "CVSS:3.1/AV:N/AC:L/PR:N/UI:R/S:U/C:H/I:N/A:N".to_string(),
                6.5,
            );
            findings.push(finding);
        }
        ...
        for issue in &analysis.cookie_analysis {
            let mut finding = Finding::new(FindingConfig {
                title: format!(
                    ...
                    issue.cookie_name, issue.issue_type
                ),
                description: format!(
                    "{}\n\nRecommendation: {}",
        ...
        if analysis.session_cookies.is_empty() && !analysis.initial_cookies.is_empty() {
            let mut finding = Finding::new(FindingConfig {
                title: "No Session Cookies Identified".to_string(),
                description: "The application returned cookies but none were identified as session cookies. This could indicate custom session management or token-based authentication.".to_string(),
                severity: Severity::Info,
