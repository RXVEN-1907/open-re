...
impl AccessControlPlugin {
                ...
                if resp.status().is_success() || resp.status().as_u16() == 400 {
                    ...
                    findings.push(self.create_finding(
                        ...
                        vec![
                            ...
                            "authorization-bypass".to_string(),
                        ],
                        vec![
                ...
                .await
            {
                if resp.status().is_success() && method != "GET" {
                    ...
                    findings.push(self.create_finding(
                        ...
                        &url,
                        vec![
                            "privilege-boundary".to_string(),
                            "cross-user-modification".to_string(),
                        ],
                        vec![
                                "Implement ownership checks for all modification operations"
                                    .to_string(),
    ...
    fn create_finding(
        ...
        let mut finding = Finding::new(FindingConfig {
            ...
            plugin_source: "access_control".to_string(),
...
impl Plugin for AccessControlPlugin {
    ...
    async fn execute(&self, request: CapabilityRequest) -> Result<CapabilityResponse> {
        ...
        // Test IDOR
        let idor_findings = self.test_idor(target_url, &auth_tokens, scan_id).await;
        all_findings.extend(idor_findings);

        // Test missing authorization
        let auth_findings = self
            .test_missing_authorization(target_url, &auth_tokens, scan_id)
            .await;
        all_findings.extend(auth_findings);

        // Test privilege boundaries
        let boundary_findings = self
            .test_privilege_boundaries(target_url, &auth_tokens, scan_id)
            .await;
        all_findings.extend(boundary_findings);

        // Test information disclosure
        let info_findings = self
            .test_information_disclosure(target_url, &auth_tokens, scan_id)
            .await;
