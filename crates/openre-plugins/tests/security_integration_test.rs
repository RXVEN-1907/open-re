...
fn create_test_context(target_url: &str) -> AnalysisContext {
    AnalysisContext {
        job_id: JobId::new(),
        project_id: openre_core::ids::ProjectId::new(),
        file_id: openre_core::ids::FileId::new(),
        function_id: None,
        block_id: None,
        instruction_id: None,
        binary_size: 0,
...
async fn test_auth_discovery_plugin_finds_login_form() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200)
            .set_body_string(r#"<form><input type="password" name="password"><input type="submit" value="Login"></form>"#)
    ...
    Mock::given(method("GET"))
        .and(path("/register"))
        .respond_with(ResponseTemplate::new(200)
            .set_body_string(r#"<form><input type="password" name="password"><input type="password" name="confirm_password"><button type="submit">Register</button></form>"#)
            .insert_header("content-type", "text/html"))
        .mount(&mock_server)
        .await;
...
async fn test_auth_discovery_plugin_detects_sso() {
    ...
    Mock::given(method("GET"))
        .and(path("/login"))
        .respond_with(ResponseTemplate::new(200)
            .set_body_string(r#"<div>Sign in with Google</div><div>Continue with GitHub</div><div>Microsoft Azure AD</div>"#)
            .insert_header("content-type", "text/html"))
        .mount(&mock_server)
        .await;
    ...
    let sso_finding = findings.iter().find(|f| {
        ...
        title.contains("SSO") || title.contains("OAuth") || title.contains("OIDC")
    });
    assert!(sso_finding.is_some());
}
...
async fn test_information_disclosure_plugin_detects_debug_page() {
    ...
    Mock::given(method("GET"))
        .and(path("/debug"))
        .respond_with(ResponseTemplate::new(200)
            .set_body_string(r#"<html><title>Debug Page</title><body>Whoops! There was an error.</body></html>"#)
            .insert_header("content-type", "text/html"))
        .mount(&mock_server)
        .await;
