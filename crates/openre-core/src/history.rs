...
use crate::ids::{FindingId, ProjectId, ScanId, TargetId, WorkflowId};
use crate::reporting::{ReportFormat, RiskLevel, ScanComparison};
use crate::result::*;
...
pub struct ReportArtifact {
    ...
    pub config: WorkflowReportConfig,
    /// Metadata
...
pub enum InvestigationStageConfig {
    /// Discovery stage configuration
    Discover(DiscoverConfig),
    /// Analysis stage configuration
    Analyze(AnalyzeConfig),
    /// Correlation stage configuration
    Correlate(CorrelateConfig),
    /// Verification stage configuration
    Verify(VerifyConfig),
    /// Prioritization stage configuration
    Prioritize(PrioritizeConfig),
    /// Reporting stage configuration
    Report(WorkflowReportConfig),
}

/// Discover stage configuration
...
pub struct AnalyzeConfig {
    /// Knowledge base to use for enrichment
    pub use_knowledge_base: bool,
    /// Enable CVE matching
    pub enable_cve_matching: bool,
    /// Enable CAPEC mapping
    pub enable_capec_mapping: bool,
    /// Enable MITRE ATT&CK mapping
    pub enable_attack_mapping: bool,
}

/// Correlate stage configuration
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
...
pub struct VerifyConfig {
    ...
    /// Only run safe (non-destructive) verifications
    pub safe_only: bool,
...
/// Workflow session for persistence
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct WorkflowSession {
    ...
    pub target: String,
    /// Associated scan ID (if any)
    pub scan_id: Option<ScanId>,
    /// Stages in the workflow
    pub stages: Vec<InvestigationStageConfig>,
    /// Current stage index
    pub current_stage_index: usize,
    /// Workflow status
    pub status: WorkflowStatus,
    /// Stage results (indexed by stage index)
...
pub struct StageResult {
    /// Stage index in workflow
    pub stage_index: usize,
    /// Stage status
    pub status: StageStatus,
    /// Stage input (serialized)
...
pub struct WorkflowArtifact {
    ...
    /// Artifact name
    pub name: String,
    /// Artifact data (serialized)
    pub data: serde_json::Value,
    /// Generated at
    pub generated_at: DateTime<Utc>,
    /// Associated stage index
    pub stage_index: usize,
}
...
impl HistoryManager {
    ...
    pub async fn list_workflow_sessions(
        ...
        self.storage.list_workflow_sessions(scan_id, status, limit, offset).await
    }
