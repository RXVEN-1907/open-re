...
impl QueueManager {
    ...
    pub async fn dequeue(
        ...
        worker_id: &str,
        priorities: &[Priority],
    ) -> OpenreResult<Option<Job>> {
        let _permit = self
            .semaphore
            .acquire()
            .await
            .map_err(|_| openre_core::Error::Internal(anyhow::anyhow!("Semaphore closed")))?;
    ...
    async fn cleanup_stale_jobs(&self) -> OpenreResult<()> {
        ...
        let stale_threshold =
            chrono::Duration::minutes(self.config.stale_job_timeout_minutes as i64);
